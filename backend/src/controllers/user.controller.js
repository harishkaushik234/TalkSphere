...
// // Allow user to update profile fields (name, bio, languages, location, avatar)
...
//     // Update Stream user if profilePic or name changed
//     if (updates.profilePic || updates.fullName) {
//       try {
...
//     // Explicitly select fields including profilePic (publicly visible)
...
// // Cancel an outgoing pending friend request (unsend)
...
export async function getFriendRequests(req, res) {
  try {
    ...
    // ✅ FILTER BROKEN RECORDS (IMPORTANT)
...
export async function getOutgoingFriendReqs(req, res) {
  try {
    ...
    outgoingRequests = outgoingRequests.filter((r) => r.recipient);
